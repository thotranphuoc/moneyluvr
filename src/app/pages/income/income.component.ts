...
@Component({
  ...
  template: `
    <h1 class="page-header">{{ 'pages.income' | translate }}</h1>
    ...
    <mat-card class="ml-card list-card">
      ...
      <mat-card-content>
        @if (transactions().length === 0) {
          <p class="empty">{{ 'income.empty' | translate }}</p>
        } @else {
          <ul class="tx-list">
...
export class IncomeComponent {
  ...
  constructor() {
    ...
    effect(() => {
      if (this.data.initialized()) this.loadTransactions();
    });
  }
  ...
  async submit(): Promise<void> {
    ...
    try {
      ...
      if (id) {
        ...
        this.snackBar.open(this.translate.instant('messages.incomeAdded'), '', { duration: 2000 });
      }
      this.cancelForm();
